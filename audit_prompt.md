...
> POC による検証とパッチ適用まで一貫して実行させるためのプロンプトテンプレートです。  
...
```
v{{VERSION}}リリース前に、以下のソースファイルすべてを精査し、監査→POC→修正→テストの全フローを実行してください。
...
- 既存テスト (`python -m pytest tests/ -x`) が全パスすることを確認
...
- `python -m pytest tests/ -x` で既存テストと合わせて全パスすること
- `python -m tox -e lint` でリントエラーがないこと
- `python -m tox -e type` で型チェックエラーがないこと
```
...
**脆弱性** のみを精査し、上記フェーズ 1〜4 を実行してください。
```
