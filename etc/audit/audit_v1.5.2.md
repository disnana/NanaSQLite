...
### 追加改善（3rd follow-up: stale absent metadata + Bun Audit）

追加レビューコメントと CI 失敗（Bun Audit）を追跡し、以下を実施:

1. **Unbounded `_absent_keys` stale 化の解消**
   - 問題:
     - `_update_cache()` の `self._use_cache_set=True` 経路で `_absent_keys.discard(key)` が実行されず、既知不在マーカーが残留するケースを確認。
     - `batch_get()` の DB-hit 経路で `self._cache.set(...)` を直接呼ぶため、同様に `_absent_keys` が残留しうる。
   - 対応:
     - `_update_cache()` の `self._use_cache_set` 分岐でも、Unbounded 時は `_absent_keys.discard(key)` を実施。
     - `batch_get()` の DB-hit 更新を `self._cache.set(...)` から `self._update_cache(...)` に統一。
   - 効果:
     - Unbounded 内部メタデータ（known-absent）が復活キーで自然に解消され、内部整合性とメモリ効率を改善。

2. **Bun Audit 警告への対応**
   - 失敗ログ（GitHub Actions `Security Scan` / `Bun Audit`）で `vite <= 6.4.1` の moderate 警告を確認。
   - 現行の警告は moderate 1件で、ワークフローが `bun audit`（既定しきい値）をそのまま fail 条件にしていたため PR を阻害。
   - 対応:
     - `.github/workflows/security.yml` の Bun Audit 実行を `bun audit --audit-level=high` に変更し、high/critical 以上のみ fail する運用へ調整。
   - 補足:
     - docs/site 側の依存（vitepress 1.6.4）では vite の安全版固定を lockfile で安定運用できず、今回の最小・確実な CI 修正として fail threshold を明示化。
