...
### 2.4 Rustによるコア処理の高速化
- SQLサニタイズ処理や文字列バリデーションをRust拡張 (`nanalib`) に移行し、10倍以上の高速化を実現します。
- **今後の目標 (SIMD最適化)**: 巨大なクエリを処理する際、SIMD (Single Instruction, Multiple Data) を用いて16/32バイト単位で高速ジャンプする実装を検討します。
