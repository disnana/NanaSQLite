...
def test_v2_engine_sharing_leak_fix(tmp_path):
    ...
    try:
        ...
        # (並列実行環境も考慮して余裕を持たせつつ、100は超えないことを確認)
        assert current_threads < initial_threads + 10, (
            f"Too many threads created: {current_threads - initial_threads}. "
            "Engines are not being shared correctly."
        )
