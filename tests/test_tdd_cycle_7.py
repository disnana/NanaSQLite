...
Issue:
  ...
  Fix: update [tool.mypy] python_version from "3.9" to "3.10" in pyproject.toml
  (Python 3.9 reached EOL October 2025; the runtime is already 3.12).
...
class TestMypyPassesWithEllipsisType:
    ...
    def test_mypy_no_name_defined_error_in_core(self):
        ...
        assert name_defined_errors == [], (
            ...
            + "\nFix: update python_version to '3.10' in pyproject.toml [tool.mypy]"
        )
    ...
    def test_mypy_python_version_in_config_is_at_least_310(self):
        """[tool.mypy] python_version must be >= '3.10' to support types.EllipsisType."""
        ...
        for line in content.splitlines():
            ...
            if in_mypy_section and line.strip().startswith("python_version"):
                # e.g. python_version = "3.10"
                version_str = line.split("=", 1)[1].strip().strip('"').strip("'")
                major, minor = map(int, version_str.split(".")[:2])
                assert (major, minor) >= (3, 10), (
                    f"[tool.mypy] python_version = '{version_str}' is too old for "
                    f"types.EllipsisType (need >= 3.10). Update to '3.10'."
                )
                return
