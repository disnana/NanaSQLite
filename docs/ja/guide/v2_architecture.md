...
通常の `v2_mode=True` は「書き込みを非同期化する」ための設定です。一方、`memory_first=True` は「CRUD の正本をメモリ側に置き、差分だけを定期的に永続化する」ための設定です。そのため、データセット全体がメモリに載る単一プロセス用途に向いています。
...
> - `fork` を使うデーモン
> - `memory_first=True` を複数プロセスから同じDBファイルに対して使う構成

### データ整合性に関する注意
- `manual` または `count` モードの場合、アプリが強制終了すると **バッファ内のデータが失われます**。
- `memory_first=True` では、最後の定期フラッシュ以降の変更差分が失われる可能性があります。
...
- [ ] `memory_first=True` を使う場合、全データがメモリに載り、重要な区切りで `flush(wait=True)` できるか？
