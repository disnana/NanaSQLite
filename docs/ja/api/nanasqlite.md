...
- `memory_first` (bool, 任意): CRUD 速度特化のメモリ優先モードを有効化します。KVS 全体をメモリへ読み込み、CRUD 操作をメモリ上で処理し、変更差分を v2 エンジンでバックグラウンドフラッシュします。デフォルトの無制限キャッシュが必要で、シングルプロセス専用です。デフォルトは `False`。(v1.5.7以降)
- `memory_flush_interval` (float, 任意): `memory_first=True` の差分フラッシュ間隔（秒）。デフォルトは `5.0`。(v1.5.7以降)
