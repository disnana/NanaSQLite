...
- `NanaSQLiteValidationError`: `dest_path` が DB ファイル自身と同一である場合（自己コピー防止）。
...
- `NanaSQLiteValidationError`: 現在の DB が `':memory:'` または `'file::memory:...'` の場合（インメモリ DB にはバックアップファイルをリストアできません）。
