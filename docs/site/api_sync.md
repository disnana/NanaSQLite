...
| `memory_first` | `bool` | True の場合、KVS 全体をメモリに読み込み、CRUD をメモリ上で処理し、差分を v2 エンジンでフラッシュする |
| `memory_flush_interval` | `float` | memory_first=True 時の差分フラッシュ間隔（秒）。デフォルト 5.0 |
