...
専用のスレッドプールにより、イベントループをブロックせずに高速なDB操作が可能です。

### 4. トランザクションとマルチテーブル
同一データベース内の複数テーブルを、接続を共有して安全に操作できます。
`with db.transaction():` による自動コミット/ロールバックもサポートしています。

### 5. Pydantic 互換性
Pydanticモデルを直接保存したり、取得したりすることができます。バリデーション済みのデータをそのままDBに永続化できます。
